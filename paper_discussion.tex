...
The label transfer experiments reveal the most compelling advantage of foundation models: superior robustness to batch effects. Our controlled comparison—contrasting intra-study transfer (no batch effects) with cross-study transfer (batch effects present)—quantifies this advantage precisely:

\begin{itemize}
    \item \textbf{Traditional methods}: Performance degrades 38\% when batch effects are introduced (ARI: 0.691 $\rightarrow$ 0.430)
    \item \textbf{SCimilarity}: Performance degrades only 20\% under the same conditions (ARI: 0.779 $\rightarrow$ 0.625)
\end{itemize}

Traditional methods lose nearly twice as much accuracy to batch effects compared to SCimilarity. This robustness advantage, combined with SCimilarity's intrinsic 13\% superiority even without batch effects, produces the dramatic 45\% improvement in cross-study settings and the 113\% gain in rare cell type macro F1.

Traditional approaches operate in gene expression space, where technical batch effects—sequencing platform differences, processing protocols, ambient RNA contamination—can dominate biological signals. A cell type signature learned from one study may fail to transfer to another due to systematic technical differences, even when the underlying biology is identical. Foundation models, trained on diverse datasets spanning multiple platforms and tissues, learn representations that are inherently more robust to these technical variations. The pre-trained SCimilarity embeddings appear to capture biological cell type identity while factoring out batch-specific technical noise, maintaining 80\% of within-study performance when applied across studies (versus 62\% for traditional methods).

Direct batch correction benchmarking (Experiment 4) provides orthogonal validation: SCimilarity achieves a standardized scib score of 0.692, competitive with scVI (0.729)—a method explicitly trained for batch correction. This is remarkable given that SCimilarity requires no dataset-specific training, unlike scVI which must be trained jointly on the datasets being integrated. Notably, SCimilarity achieved the highest biological conservation score (0.789) across all methods, indicating superior preservation of cell type structure while maintaining competitive batch mixing. This profile—strong biological conservation coupled with inherent batch robustness—is precisely what meta-analyses and atlas construction require.
